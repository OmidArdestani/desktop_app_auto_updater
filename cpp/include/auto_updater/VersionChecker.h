 ...
 *        whether a newer version is available.
 *
 * Mirrors the Python LocalStorageVersionChecker class exactly.
 *
 * Expected JSON manifest format:
 * @code{.json}
 * {
 *   "version":       "2.1.0",
 *   "download_url":  "\\192.168.10.11\public_storage\the_application_\app-2.1.0-setup.exe",
 *   "checksum":      "e3b0c44298fc1c149afbf4c8996fb924...",
 *   "release_notes": "Bug fixes and performance improvements.",
 *   "mandatory":     false,
 *   "file_name":     "app-2.1.0-setup.exe"
 ...
 * @param timeoutMs    Request timeout in milliseconds. Defaults to 10 000.
 */
class LocalStorageVersionChecker : public IVersionChecker
{
public:
    explicit LocalStorageVersionChecker(const QString &manifestUrl, int timeoutMs = 10000)
        : m_manifestUrl(manifestUrl)
        , m_timeoutMs(timeoutMs)
        {}
